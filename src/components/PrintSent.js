...
export default function PrintReceived() {
  ...
  const searchData = () => {
    Axios.post('http://localhost:3080/api/filter/sent', {
      startDate: startDate,
      endDate: endDate,
    })
      .then((response) => {
    ...
    <div className="container mx-auto p-8">
      <div className="flex justify-center gap-4">
        ...
        <div className="flex flex-col justify-end">
          <button
            className="bg-gray-500 hover:bg-gray-600 focus:outline-none focus:ring focus:ring-blue-300 text-sm text-gray-50 font-bold rounded-lg shadow shadow-gray-500/50 p-2"
            onClick={searchData}
          >
            Cari
          </button>
              ...
              <tbody className="bg-white border border-black">
                {sentList.map((value) => {
                  return (<tr key={value.id}>
                      ...
                      <td className="text-xs text-left text-black font-medium uppercase tracking-wider border border-black py-3 px-6">{value.receiver}</td>
                    </tr>
                )})}
              </tbody>
            </table>
